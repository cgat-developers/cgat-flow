...
class FeatureCountsQuantifier(Quantifier):
    ...
    def run_featurecounts(self, level="gene_id"):
        ...
        if BamTools.is_paired(bamfile):
            # sort bamfile
            tmp_dir = P.get_temp_dir()
            bam_tmp = os.path.join(tmp_dir, os.path.basename(bamfile))
        ...
        statement = '''mkdir %(tmp_dir)s;
                       zcat %(annotations)s > %(tmp_dir)s/geneset.gtf;
                       %(paired_processing)s
                       featureCounts %(options)s
                                     -T %(job_threads)i
                                     -s %(strand)s
                                     -a %(tmp_dir)s/geneset.gtf
                                     %(paired_options)s
