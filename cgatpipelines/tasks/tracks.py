'''tracks.py - Working with sample names in pipelines
=============================================================
...
:mod:`tracks` module assists in controlling these data
...
There are three class within :mod:`tracks`: :class:`Sample`,
...
To build aggregates, use :class:`tracks.Aggregate`. The
...
require all conditions or all tissues::
   ...
   CONDITIONS = tracks.Aggregate(TRACKS, labels=("condition",))
   TISSUES = tracks.Aggregate(TRACKS, labels=("tissue",))
...
``liver-stimulated-agg``
    ...
    >>> replicates = tracks.getSamplesInTrack( track, TRACKS )
    >>> print replicates
