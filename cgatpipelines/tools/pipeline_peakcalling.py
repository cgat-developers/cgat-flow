"""=========================================================
pipeline_peakcalling - Produce Peaklists from Bam Files
=========================================================
...
if PARAMS['downsampling_downsample_bams'] == 1:
    @follows(filtering)
    @follows(mkdir("downsampled_bams.dir"))
        ...
        if PARAMS['downsampling_downsample_method'] == 'specific_number':
            ...
            if PARAMS['downsampling_downsample_method'] == 'smallest_overall_bam':
                ...
                for item in PARAMS['downsampling_group_samples_for_downsampling_on']:
                    ...
                    if count == 0:
                        # subset dataframe based on tissue, factor, condition, treatment
    ...
    if PARAMS['quant_norm'] == 1:
        for idx in idxstats:
            ...
            if file_name == inf_name:
                ...
                scale = peakcalling.getSpikeInReads(idx, regex)
                contig_sizes = peakcalling.getContigSizes(idx)
            else:
                continue
...
if PARAMS.get('IDR_run', False):
    ...
    def makePooledInputs(infiles, outfiles):
        ...
        for ct in cond_tissues:
            ...
            # inputs
            peakcalling.mergeSortIndex(inputs, out)
...
if PARAMS.get('IDR_run', False):
    ...
    def makePseudoBams(infiles, outfiles):
        ...
        peakcalling.makeBamLink(infile, orig)

        peakcalling.makePseudoBams(infile, pseudos,
                                           PARAMS['paired_end'],
    ...
    def makePseudoBams(infile, outfile):
        ...
        '''
        peakcalling.makeBamLink(infile[0], outfile)
...
if PARAMS['IDR_poolinputs'] == "none":
    ...
    def makeIDRInputBams(infile, outfile):
        ...
        '''
        infile = infile[0]
        peakcalling.makeBamLink(infile, outfile)
...
def estimateInsertSize(infile, outfile):
    ...
    peakcalling.estimateInsertSize(infile,
                                           outfile,
                                           PARAMS['paired_end'],
...
def callMacs2peaks(infiles, outfile):
    ...
    peakcaller = peakcalling.Macs2Peakcaller(
        threads=1,
...
def callNarrowerPeaksWithSicer(infiles, outfile):
    ...
    peakcaller = peakcalling.SicerPeakcaller(
        threads=1,
...
def runIDR(infile, outfile):
    ...
    statement = peakcalling.buildIDRStatement(
        infile1, infile2,
        T,
        idrthresh,
...
def summariseIDR(infiles, outfile):
    ...
    peakcalling.summariseIDR(infiles, outfile, pooledc, selfc, repc)
...
def runIDRQC(infile, outfile):
    peakcalling.doIDRQC(infile, outfile)
...
def findConservativePeaks(infile, outfiles):
    ...
    for peakfile in peakfiles:
        ...
        peakcalling.makeLink(peakfile, outnam)
