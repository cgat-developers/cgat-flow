...
# @P.add_doc(windows.convertReadsToIntervals)
...
def prepareTags(infile, outfile):
    ...
    '''
    windows.convertReadsToIntervals(
        infile,
        outfile,
...
# @P.add_doc(windows.countTagsWithinWindows
...
def countTagsWithinWindows(infiles, outfile):
    ...
    windows.countTagsWithinWindows(
        bedfile,
        windowfile,
        outfile,
...
# @P.add_doc(windows.aggregateWindowsTagCounts)
...
def aggregateWindowsTagCounts(infiles, outfile):
    ...
    windows.aggregateWindowsTagCounts(infiles,
                                              outfile,
                                              regex="(.*).counts.bed.gz")
...
def countTagsWithinContext(infiles, outfile):
    ...
    windows.countTagsWithinWindows(tagfile,
                                           windowfile,
                                           outfile,
...
# @P.add_doc(windows.normalizeBed)
...
def normalizeBed(infile, outfile):
    ...
    P.submit(module='cgatpipelines.tasks.windows',
...
# @P.add_doc(windows.enrichmentVsInput)
...
def runFilterAnalysis(infiles, outfile):
    ...
    '''
    windows.outputRegionsOfInterest(
        infiles[0],
        infiles[1],
        outfile)


# @P.add_doc(windows.runMEDIPSDMR)
...
def runMedipsDMR(infile, outfile):
    ...
    '''
    windows.runMEDIPSDMR(infile, outfile)
...
# @P.add_doc(windows.buildSpikeResults)
@jobs_limit(PARAMS.get("jobs_limit_db", 1), "db")
...
def buildSpikeResults(infile, outfile):
    ...
    '''
    windows.buildSpikeResults(infile, outfile)
...
# @P.add_doc(windows.buildDMRStats)
...
def buildDMRStats(infile, outfile):
    ...
    windows.buildDMRStats([infile], outfile, method=method)


# @P.add_doc(windows.plotDETagStats)
...
def plotDETagStats(infiles, outfile):
    ...
    windows.plotDETagStats(
        infiles[0], infiles[1], outfile,
        submit=True,
        job_options="-l mem_free=16")


# @P.add_doc(windows.buildFDRStats)
...
def buildFDRStats(infile, outfile):
    ...
    method = os.path.dirname(infile)
    method = P.snip(method, ".dir")
    windows.buildFDRStats(infile, outfile, method=method)


# @P.add_doc(windows.outputAllWindows)
@transform(mergeDMRWindows,
...
def outputAllWindows(infile, outfile):
    ...
    '''
    windows.outputAllWindows(infile, outfile)
...
# @P.add_doc(windows.mergeSummarizedContextStats)
...
def mergeSummarizedContextStats(infiles, outfile):
    ...
    """
    windows.mergeSummarizedContextStats(infiles, outfile)
...
# @P.add_doc(windows.normalizeTagCounts)
...
def runMedipsQC(infile, outfile):
    '''run MEDIPS single file analysis
    '''
    windows.runMEDIPSQC(infile, outfile)
