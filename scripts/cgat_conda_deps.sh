...
sfood --ignore=pipeline_metagenomeassembly \
 --ignore=pipeline_metagenomebenchmark \
 ${REPO_FOLDER}/$1 2>&1 \
 | grep 'WARNING     :   ' \
 | grep -v Line \
 | awk '{print $3;}' \
 | grep -v '^_.*' \
 | sed 's/\..*//g' \
 | egrep -v 'urllib|CGATPipelines|PipelineTracks|RnaseqDiffExpressionReport|titrationReport' \
 | sort -u \
 >> ${TMP_SFOOD}
...
sfood --ignore=pipeline_metagenomeassembly \
 --ignore=pipeline_metagenomebenchmark \
 -f ${REPO_FOLDER}/CGATPipelines/$1 2>&1 \
 | grep 'WARNING     :   ' \
 | grep -v Line \
 | awk '{print $3;}' \
 | grep -v '^_.*' \
 | sed 's/\..*//g' \
 | grep -v 'urllib|CGATPipelines|PipelineTracks|RnaseqDiffExpressionReport|titrationReport' \
